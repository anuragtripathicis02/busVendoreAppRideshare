...
const UploadBox = ({
  ...
  return (
    <div className="form-group mb-24">
      <Form.Label className="fw-medium mb-2">{label}</Form.Label>
      <div
        onClick={triggerUpload}
        onDrop={handleDrop}
        onDragOver={(e) => e.preventDefault()}
        className="border-upload d-flex align-items-center justify-content-center"
        style={{
          height: '160px',
          width: '100%',
          minWidth: aspect === 'square' ? '160px' : undefined,
          aspectRatio: aspect === 'square' ? '1 / 1' : '3 / 1',
          cursor: 'pointer',
          overflow: 'hidden',
        }}
      >
...
const CompanyInfoForm: React.FC<Props> = ({ nextStep }) => {
  const handleLogoIcon = (file: File) => {
    console.log('Logo Icon:', file);
  };

  const handleFullLogo = (file: File) => {
    console.log('Full Logo:', file);
  };
  ...
  return (
    <div className="card-box">
      <Form>
        <Row>
          <Col lg={4} md={6}>
            <UploadBox label="Logo Icon" aspect="square" onChange={handleLogoIcon} />
          </Col>
          <Col lg={8} md={6}>
            <UploadBox label="Full Size Logo" aspect="wide" onChange={handleFullLogo} />
          </Col>
        </Row>
